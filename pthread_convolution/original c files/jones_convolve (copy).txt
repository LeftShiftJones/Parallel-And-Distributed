...
#include "lodepng.h" //Png Loading application
...
{
    unsigned int error = lodepng_encode32_file(&image->pixels, &image->columns, &image->rows, file_name);
    if (error)
    {
        fprintf(stderr, "error %u: %s\n", error, lodepng_error_text(error));
    }
...
#define ERR_MSGE_LEN 128
    ...
    //put image splitting logic here
